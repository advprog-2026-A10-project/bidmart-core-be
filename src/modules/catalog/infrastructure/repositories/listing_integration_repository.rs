...
impl ListingIntegrationPort for PostgresListingIntegrationRepository {
    async fn get_listing_status(&self, id: Uuid) -> Result<Option<ListingStatus>, ListingError> {
        sqlx::query_scalar::<_, ListingStatus>("SELECT status FROM listings WHERE id = $1")
            .bind(id)
    ...
    async fn update_listing_status(
        ...
        let affected =
            sqlx::query("UPDATE listings SET status = $2, updated_at = NOW() WHERE id = $1")
                .bind(id)
                .bind(status)
                .execute(&self.pool)
                .await
    ...
    async fn update_ends_at(
        ...
        let affected =
            sqlx::query("UPDATE listings SET ends_at = $2, updated_at = NOW() WHERE id = $1")
                .bind(listing_id)
                .bind(new_ends_at)
                .execute(&self.pool)
                .await
