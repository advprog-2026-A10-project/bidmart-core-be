...
/// Per-module request tracer. Logs one structured line at the start of every
/// request and another when it completes (with status + elapsed). The
/// `module` field lets `RUST_LOG=core_be.catalog.request=info,...` filter
/// per-module traffic, and the `request_id` field — taken from
/// `X-Request-ID` when present or generated otherwise — is echoed back on the
/// response so callers can correlate logs end-to-end.
pub async fn request_trace(req: Request, next: Next) -> Response {
    ...
    tracing::info!(
        target: "core_be.catalog.request",
